...
url:"../../Public/Images/",//image location handle.
        ...
        else
        {
              if(mem.grid[idx] == mem.grid[mem.last])
              {
                    ...
                    if(mem.matches.length == mem.sets * 2)
                    {
                          ...
                          winn.innerHTML = "you have won!! Total moves :: " + mem.moves +"<br>";
                          var node = document.createElement("button");
                          node.id = 'resetButton';
                          node.innerHTML = "Reset Game";
                          winn.appendChild(node);
                          node = document.getElementById("resetButton");
                          node.onclick = function ()
                          {     
                                node = document.getElementById("win-text");
